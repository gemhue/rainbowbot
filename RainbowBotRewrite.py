            ...
            if yesorno.value == True:
                ...
                if yesorno.method == "Recent":
                    ...
                    purged_message = discord.Embed(
                        ...
                        description=f"{user.mention} has just purged {len(deleted)} messages from {channel.mention} using the `/purge here` command!"
                    )
                    await channel.send(embed=purged_message)
        ...
        try:
            ...
            if csv.value == True:
                ...
                if yon.value == True:
                    ...
                    for channel in csv.channels:
                        ...
                        if yon.method == "Recent":
                            ...
                            purged_message = discord.Embed(
                                color=Colour.blurple(),
                                title="Messages Purged",
                                description=f"{user.mention} has just purged {len(deleted)} messages from {channel.mention} using the `/purge channels` command!"
                            )
                            await channel.send(embed=purged_message)
                        elif yon.method == "All":
                            deleted = await channel.purge(limit=1000, check=lambda m: m.pinned == False, oldest_first=True, bulk=True)
                    ...
                    success = discord.Embed(color=Colour.green(), title="Success", description=f'The purge is now complete!')
                    await response.edit(embed=success, view=None)
                    await response.delete(delay=10.0)

                elif yon.value == False:
                    cancelled = discord.Embed(color=Colour.red(), title="Cancelled", description='This interaction has been cancelled. No messages have been purged.')
                    await response.edit(embed=cancelled, view=None)
                    await response.delete(delay=10.0)

                else:
                    timed_out = discord.Embed(color=Colour.yellow(), title="Timed Out", description='This interaction has timed out. No messages have been purged.')
                        ...
                        for channel in purge_channels:
                            ...
                            if yon.method == "Recent":
                                ...
                                purged_message = discord.Embed(
                                    color=Colour.blurple(),
                                    title="Messages Purged",
                                    description=f"{user.mention} has just purged {len(deleted)} messages from {channel.mention} using the `/purge server` command!"
                                )
                                await channel.send(embed=purged_message)
                            elif yon.method == "All":
                                deleted = await channel.purge(limit=1000, check=lambda m: m.pinned == False, oldest_first=True, bulk=True)

...
class BackgroundTasks(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        try:
            ...
            cur = await self.db.execute("SELECT welcome_channel_id FROM guilds WHERE guild_id = ?", (guild_id,))
            ...
            cur = await self.db.execute("SELECT join_role_id FROM guilds WHERE guild_id = ?", (guild_id,))
            row = await cur.fetchone()
            role_id = row[0]
            ...
            cur = await self.db.execute("SELECT bot_role_id FROM guilds WHERE guild_id = ?", (guild_id,))
            row = await cur.fetchone()
            botrole_id = row[0]
    ...
    async def on_member_remove(self, member: discord.Member):
        try:
            ...
            cur = await self.db.execute("SELECT goodbye_channel_id FROM guilds WHERE guild_id = ?", (guild_id,))
            row = await cur.fetchone()
            channel_id = row[0]
            if channel_id is not None:

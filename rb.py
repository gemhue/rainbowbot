        ...
        if view.value == True:
            ...
            for channel in purgechannels:
                ...
                while len(unpinned) > 0:
                    deleted += await channel.purge(check=lambda message: message.author == ctx.user and message.pinned == False, oldest_first=True)
                if len(deleted) == 1:
...
async def setup(bot):
    ...
    await bot.add_cog(AwardCommands(bot))
